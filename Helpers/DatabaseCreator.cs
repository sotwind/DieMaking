...
public static class DatabaseCreator
{
    ...
    /// 异步确保数据库存在
    /// </summary>
    public static async Task<DatabaseEnsureResult> EnsureDatabaseExistsAsync(CancellationToken cancellationToken = default)
    {
        var databaseName = GetDatabaseName();
        ...
        try
        {
            ...
            var createSql = $@"
                CREATE DATABASE [{databaseName}]
                COLLATE Chinese_PRC_CI_AS
                ON PRIMARY (NAME = N'{databaseName}', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\{databaseName}.mdf', SIZE = 10MB, MAXSIZE = UNLIMITED, FILEGROWTH = 10MB)
                LOG ON (NAME = N'{databaseName}_log', FILENAME = N'C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\{databaseName}_log.ldf', SIZE = 5MB, MAXSIZE = 2GB, FILEGROWTH = 5MB)";
