...
public class DieService
{
    ...
    /// 检查刀模编号是否存在
    /// </summary>
    public bool IsDieCodeExists(string dieCode)
    {
        ...
        const string sql = "SELECT COUNT(1) FROM DieInfo WHERE DieCode = @DieCode";
    ...
    public (bool IsInStock, DateTime? InStockTime, string? LocationCode) GetDieStockStatus(int dieId)
    {
        ...
        if (reader.Read())
        {
            var inStockTime = reader["InStockTime"] == DBNull.Value ? null : Convert.ToDateTime(reader["InStockTime"]);
